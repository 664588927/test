...
public class XRefreshView extends LinearLayout {
     ...
     * header 的显示模式
     */
    public static final int TEXT_PROGRESS_TYPE = 0;
    public static final int TEXT_ANIMATION_TYPE = 1;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                int childWidthSpec = MeasureSpec.makeMeasureSpec(getMeasuredWidth()
                                - lp.leftMargin - lp.rightMargin - paddingLeft - paddingRight,
                        MeasureSpec.EXACTLY);
...
//        mFooterCallBack.onStateRefreshing();
    ...
    private Runnable mRunnable = new Runnable() {
        ...
        public void run() {
            if (mScroller.computeScrollOffset()) {
                ...
                if (enableReleaseToLoadMore && mHolder.mOffsetY == 0 && mReleaseToLoadMore
                        && mContentView != null && mContentView.isBottom()) {
    ...
    public void setHeaderType(int headerType) {
        ((XRefreshViewHeader) mHeaderView).changedType(headerType);
    }
